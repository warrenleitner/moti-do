...
This module contains tests for the config functions, including:
- get_config_path: Returns the expected path for the config file
- load_config: Loads the configuration from the config file
- save_config: Saves the configuration to the config file
"""
...
def test_load_config_file_not_exists(_mock_exists: Any, _mock_get_path: Any) -> None:
    """Test loading config when the file doesn't exist."""
    config = config_module.load_config()
    assert config == {"backend": DEFAULT_BACKEND}
    _mock_exists.assert_called_once_with(MOCK_CONFIG_PATH)


@patch("motido.data.config.get_config_path", return_value=MOCK_CONFIG_PATH)
@patch("motido.data.config.os.path.exists", return_value=True)
def test_load_config_valid_json_backend(_mock_exists: Any, _mock_get_path: Any) -> None:
...
@patch("motido.data.config.get_config_path", return_value=MOCK_CONFIG_PATH)
@patch("motido.data.config.os.path.exists", return_value=True)
def test_load_config_valid_db_backend(_mock_exists: Any, _mock_get_path: Any) -> None:
...
def test_load_config_invalid_json(
    mock_print: Any, _mock_exists: Any, _mock_get_path: Any
) -> None:
...
def test_save_config_success(
    mock_print: Any, mock_json_dump: Any, _mock_get_path: Any
) -> None:
    ...
    _mock_get_path.assert_called_once()
...
@patch("motido.data.config.get_config_path", return_value=MOCK_CONFIG_PATH)
@patch("motido.data.config.print")  # Mock print to check error
def test_save_config_io_error(mock_print: Any, _mock_get_path: Any) -> None:
