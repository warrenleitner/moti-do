...
from motido.data.database_manager import DatabaseDataManager, DEFAULT_USERNAME, DB_NAME
...
def mock_db_path(mocker):
    ...
    mocker.patch.object(DatabaseDataManager, '_get_db_path', return_value=mock_path, autospec=True)
...
def mock_conn_fixture(mocker):
    ...
    mock_get_conn = mocker.patch.object(DatabaseDataManager, '_get_connection', return_value=mock_connection, autospec=True)
...
def test_get_connection_success(manager, mocker, mock_db_path):
    ...
    mock_ensure_dir = mocker.patch.object(manager, '_ensure_data_dir_exists', autospec=True)
...
def test_initialize_connection_error(manager, mocker, capsys):
    ...
    mocker.patch.object(DatabaseDataManager, '_get_connection', 
...
def test_load_user_not_found(manager, mock_conn_fixture):
    ...
    cursor.execute.assert_called_once_with("SELECT username FROM users WHERE username = ?", (username,))
...
def test_load_user_db_error(manager, mock_conn_fixture, capsys):
    ...
    assert loaded_user is None
    captured = capsys.readouterr()
    assert f"Error loading user '{username}' from motido.database: Query failed" in captured.out
...
def test_save_user_new(manager, mocker, mock_conn_fixture, sample_user_db):
    ...
    mock_ensure_user = mocker.patch.object(manager, '_ensure_user_exists', autospec=True)
    ...
    cursor.execute.assert_called_once_with(
        "DELETE FROM tasks WHERE user_username = ?", 
        (sample_user_db.username,)
    )
    expected_task_data = [
...
def test_save_user_no_tasks(manager, mocker, mock_conn_fixture):
    ...
    cursor.execute.assert_called_once_with(
        "DELETE FROM tasks WHERE user_username = ?", 
        (user_no_tasks.username,)
    )
    cursor.executemany.assert_not_called()

def test_save_user_db_error_on_delete(manager, mocker, mock_conn_fixture, sample_user_db, capsys):
    ...
    cursor.executemany.assert_not_called() 
    captured = capsys.readouterr()
    assert f"Error saving user '{sample_user_db.username}' to database: Delete failed" in captured.out

def test_save_user_db_error_on_insert(manager, mocker, mock_conn_fixture, sample_user_db, capsys):
    ...
    captured = capsys.readouterr()
    assert f"Error saving user '{sample_user_db.username}' to database: Insert failed" in captured.out
