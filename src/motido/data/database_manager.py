    ...
    def load_user(self, username: str = DEFAULT_USERNAME) -> User | None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("SELECT id, description FROM tasks WHERE user_username = ?", (username,))
                task_rows = cursor.fetchall()
                tasks = [Task(id=row['id'], description=row['description']) for row in task_rows]
    ...
    def save_user(self, user: User):
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("DELETE FROM tasks WHERE user_username = ?", (user.username,))
                ...
                if tasks_to_insert:
                    ...
                    print(f"Inserted {len(tasks_to_insert)} tasks for user '{user.username}'.")
                else:
