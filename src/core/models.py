...
class User:
    ...
    def find_task_by_id(self, task_id_prefix: str) -> Task | None:
        ...
        """
        matching_tasks = [task for task in self.tasks if task.id.startswith(task_id_prefix)]
        if len(matching_tasks) == 1:
            return matching_tasks[0]
        elif len(matching_tasks) > 1:
            raise ValueError(f"Ambiguous ID prefix '{task_id_prefix}'. Multiple tasks found.")
        else:
            return None
